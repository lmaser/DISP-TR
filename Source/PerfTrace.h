...
// ============================================================================
// PerfTrace.h — Lightweight per-block performance tracing for DISP-TR
//
// Usage:
//   #define DISPTR_PERF_TRACE 1   // enable tracing (0 = no overhead)
//
// In processBlock:
//   PERF_BLOCK_BEGIN();
//   ... your DSP ...
//   PERF_BLOCK_END(perfTrace, numSamples);
//
// To dump accumulated data:
//   perfTrace.dumpToFile ("C:\\path\\to\\perf_dump.csv");
//
// The tracer stores the last N blocks in a lock-free ring buffer.
// No allocations on the audio thread. No mutex. Fully RT-safe.
// ============================================================================
...
#ifndef DISPTR_PERF_TRACE
 #define DISPTR_PERF_TRACE 0
#endif

#if DISPTR_PERF_TRACE

struct PerfBlockEntry
{
    double   timestampSec;       // wall-clock time (seconds)
    double   durationUs;         // block processing time (microseconds)
    int      blockSize;          // numSamples
    float    cpuPercent;         // estimated CPU% for this block
};

class PerfTrace
{
public:
    static constexpr int kRingSize = 8192;  // must be power-of-2

    PerfTrace() = default;

    // Called from audio thread — no allocations, no locks
    ...
    float getPeakCpuPercent() const noexcept
    {
    ...
    {
        peakCpuPercent.store (0.0f, std::memory_order_relaxed);
    }

    // Called from UI or message thread — writes CSV to disk
    ...
    // Quick summary string for debug overlay
    ...
    // Auto-dump path.  Set via setAutoDumpPath() — when non-empty, the
    // destructor will write the CSV automatically.
    ...
    // Convenience: set auto-dump to user's Desktop
    void enableDesktopAutoDump (const juce::String& filename = "disptr_perf_dump.csv")
    {
...
#else // DISPTR_PERF_TRACE == 0

// Stub: zero overhead when tracing is disabled
class PerfTrace
{
public:

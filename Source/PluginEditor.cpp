...
juce::Rectangle<float> DisperserAudioProcessorEditor::DualMixBarComponent::getInnerArea() const
{
    return getLocalBounds().toFloat().reduced (kPad);
}

DisperserAudioProcessorEditor::DualMixBarComponent::DragTarget
DisperserAudioProcessorEditor::DualMixBarComponent::hitTestMarker (juce::Point<float> p) const
{
...
void DisperserAudioProcessorEditor::DualMixBarComponent::setLevelFromMouseX (float mouseX, DragTarget target)
{
    ...
    const char* paramId = (target == DRY) ? DisperserAudioProcessor::kParamDryLevel
...
void DisperserAudioProcessorEditor::DualMixBarComponent::updateFromProcessor()
{
    ...
    const float newDry = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamDryLevel)->load();
    const float newWet = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamWetLevel)->load();
...
void DisperserAudioProcessorEditor::DualMixBarComponent::paint (juce::Graphics& g)
{
...
void DisperserAudioProcessorEditor::DualMixBarComponent::mouseDown (const juce::MouseEvent& e)
{
...
void DisperserAudioProcessorEditor::DualMixBarComponent::mouseDrag (const juce::MouseEvent& e)
{
    if (currentDrag_ != None)
    {
        setLevelFromMouseX (e.position.x, currentDrag_);
...
void DisperserAudioProcessorEditor::DualMixBarComponent::mouseUp (const juce::MouseEvent&)
{
    currentDrag_ = None;
}
...
juce::Rectangle<float> DisperserAudioProcessorEditor::FilterBarComponent::getInnerArea() const
{
    return getLocalBounds().toFloat().reduced (kPad);
}

float DisperserAudioProcessorEditor::FilterBarComponent::freqToNormX (float freq) const
{
    const float clamped = juce::jlimit (kMinFreq, kMaxFreq, freq);
    return std::log2 (clamped / kMinFreq) / std::log2 (kMaxFreq / kMinFreq);
}

float DisperserAudioProcessorEditor::FilterBarComponent::normXToFreq (float normX) const
{
    const float n = juce::jlimit (0.0f, 1.0f, normX);
    return kMinFreq * std::pow (2.0f, n * std::log2 (kMaxFreq / kMinFreq));
}

float DisperserAudioProcessorEditor::FilterBarComponent::getMarkerScreenX (float freq) const
{
    const auto inner = getInnerArea();
    return inner.getX() + freqToNormX (freq) * inner.getWidth();
}

DisperserAudioProcessorEditor::FilterBarComponent::DragTarget
DisperserAudioProcessorEditor::FilterBarComponent::hitTestMarker (juce::Point<float> p) const
{
...
void DisperserAudioProcessorEditor::FilterBarComponent::setFreqFromMouseX (float mouseX, DragTarget target)
{
    ...
    if (target == HP)
    {
        const float otherFreq = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamFilterLpFreq)->load();
        freq = juce::jmin (freq, otherFreq);
    }
    else
    {
        const float otherFreq = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamFilterHpFreq)->load();
        freq = juce::jmax (freq, otherFreq);
    }

    const char* paramId = (target == HP) ? DisperserAudioProcessor::kParamFilterHpFreq
...
void DisperserAudioProcessorEditor::FilterBarComponent::paint (juce::Graphics& g)
{
    const auto r = getLocalBounds().toFloat();
...
void DisperserAudioProcessorEditor::FilterBarComponent::mouseDown (const juce::MouseEvent& e)
{
...
void DisperserAudioProcessorEditor::FilterBarComponent::mouseDrag (const juce::MouseEvent& e)
{
    if (currentDrag_ != None)
    {
        setFreqFromMouseX (e.position.x, currentDrag_);
...
void DisperserAudioProcessorEditor::FilterBarComponent::mouseUp (const juce::MouseEvent&)
{
    currentDrag_ = None;
}
...
void DisperserAudioProcessorEditor::FilterBarComponent::mouseDoubleClick (const juce::MouseEvent& e)
{
    ...
    if (target == HP)
    {
        if (auto* param = proc.apvts.getParameter (DisperserAudioProcessor::kParamFilterHpOn))
        {
    ...
    {
        if (auto* param = proc.apvts.getParameter (DisperserAudioProcessor::kParamFilterLpOn))
        {
            const bool current = param->getValue() > 0.5f;
            param->setValueNotifyingHost (current ? 0.0f : 1.0f);
        }
...
{
    const std::array<BarSlider*, 14> barSliders { &freqSlider, &modSlider, &feedbackSlider, &amountSlider, &seriesSlider, &shapeSlider, &variationSlider, &styleSlider, &inputSlider, &outputSlider, &tiltSlider, &panSlider, &mixSlider, &limThresholdSlider };
    ...
    variationSlider.setNumDecimalPlacesToDisplay (1);
    ...
    bindSlider (variationAttachment, DisperserAudioProcessor::kParamVariation, variationSlider, kDefaultVariation);
...
void DisperserAudioProcessorEditor::sliderValueChanged (juce::Slider* slider)
{
    ...
    {
        return s == &amountSlider || s == &seriesSlider || s == &freqSlider || s == &shapeSlider || s == &variationSlider || s == &styleSlider || s == &feedbackSlider || s == &modSlider || s == &inputSlider || s == &outputSlider || s == &mixSlider;
    };
...
void DisperserAudioProcessorEditor::setPromptOverlayActive (bool shouldBeActive)
{
    ...
    const std::array<juce::Component*, 13> interactiveControls {
        &amountSlider, &seriesSlider, &freqSlider, &shapeSlider, &variationSlider, &styleSlider, &feedbackSlider, &modSlider, &inputSlider, &outputSlider, &mixSlider, &altButton, &midiButton
    };
...
void DisperserAudioProcessorEditor::timerCallback()
{
    ...
    if (crtEnabled && w > 0 && h > 0)
    {
        ...
        const bool anySliderDragging = amountSlider.isMouseButtonDown()
                                    ...
                                    || variationSlider.isMouseButtonDown()
...
bool DisperserAudioProcessorEditor::refreshLegendTextCache()
{
    ...
    const double variationV = juce::jlimit (0.0, 1.0, variationSlider.getValue());
    const int variationPct = (int) std::lround (variationV * 100.0);
    ...
    const auto oldVariationFullLen = cachedVariationTextFull.length();
    const auto oldVariationShortLen = cachedVariationTextShort.length();
    ...
    cachedVariationTextFull = juce::String (variationPct).toUpperCase() + "% VAR";
    cachedVariationTextShort = juce::String (variationPct).toUpperCase() + "% VAR";
    cachedVariationIntOnly = juce::String (variationPct);
    ...
    const bool lengthChanged = oldAmountFullLen  != cachedAmountTextFull.length()
                            ...
                            || oldVariationFullLen  != cachedVariationTextFull.length()
                            || oldVariationShortLen != cachedVariationTextShort.length()
...
void DisperserAudioProcessorEditor::openNumericEntryPopupForSlider (juce::Slider& s)
{
    ...
    const bool isPercentPrompt = (&s == &shapeSlider || &s == &variationSlider || &s == &feedbackSlider || &s == &mixSlider || &s == &panSlider);
    ...
    else if (&s == &variationSlider) { suffix = " % VAR";   suffixShort = " %"; }
    ...
    if (auto* te = aw->getTextEditor ("val"))
    {
        ...
        else if (&s == &variationSlider) worstCaseText = "100.0";
    ...
    aw->enterModalState (true,
        juce::ModalCallbackFunction::create ([safeThis, sliderPtr, aw] (int result) mutable
        {
            ...
            if (safeThis != nullptr && (sliderPtr == &safeThis->shapeSlider || sliderPtr == &safeThis->variationSlider || sliderPtr == &safeThis->feedbackSlider || sliderPtr == &safeThis->mixSlider || sliderPtr == &safeThis->panSlider))
                v *= 0.01;
...
void DisperserAudioProcessorEditor::openFilterPrompt()
{
    ...
    const float hpFreq = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamFilterHpFreq)->load();
    const float lpFreq = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamFilterLpFreq)->load();
    const int hpSlope  = juce::roundToInt (proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamFilterHpSlope)->load());
    const int lpSlope  = juce::roundToInt (proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamFilterLpSlope)->load());
    const bool hpOn    = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamFilterHpOn)->load() > 0.5f;
    const bool lpOn    = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamFilterLpOn)->load() > 0.5f;
    ...
    auto pushParams = [safeThis, hpToggle, lpToggle, hpSlopeVal, lpSlopeVal, normToFreq, aw] ()
    {
        ...
        if (lpTe) setP (DisperserAudioProcessor::kParamFilterLpFreq, lpF);
        setP (DisperserAudioProcessor::kParamFilterHpSlope, (float) *hpSlopeVal);
        setP (DisperserAudioProcessor::kParamFilterLpSlope, (float) *lpSlopeVal);

        if (auto* hpOnParam = p.apvts.getParameter (DisperserAudioProcessor::kParamFilterHpOn))
            hpOnParam->setValueNotifyingHost (hpToggle->getToggleState() ? 1.0f : 0.0f);
        if (auto* lpOnParam = p.apvts.getParameter (DisperserAudioProcessor::kParamFilterLpOn))
    ...
    aw->enterModalState (true,
             ...
             hpCyclerGuard, lpCyclerGuard, hpFwdGuard, lpFwdGuard] (int result)
        {
            ...
            if (result != 1)
            {
                ...
                setP (DisperserAudioProcessor::kParamFilterHpFreq, origHpFreq);
                setP (DisperserAudioProcessor::kParamFilterLpFreq, origLpFreq);
                setP (DisperserAudioProcessor::kParamFilterHpSlope, (float) origHpSlope);
                setP (DisperserAudioProcessor::kParamFilterLpSlope, (float) origLpSlope);
                if (auto* hpOnParam = p.apvts.getParameter (DisperserAudioProcessor::kParamFilterHpOn))
...
void DisperserAudioProcessorEditor::openMixSendPrompt()
{
    ...
    auto& proc = audioProcessor;
    const float curDry = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamDryLevel)->load();
    const float curWet = proc.apvts.getRawParameterValue (DisperserAudioProcessor::kParamWetLevel)->load();
    ...
    struct PromptBar : public juce::Component
    {
        ...
        PromptBar (const DISPScheme& s, float initial, float def)
    ...
    auto* dryBar = new PromptBar (scheme, curDry, DisperserAudioProcessor::kDryLevelDefault);
    auto* wetBar = new PromptBar (scheme, curWet, DisperserAudioProcessor::kWetLevelDefault);
    ...
    auto pushParams = [safeThis, aw, dbToLinear] ()
    {
        ...
        setP (DisperserAudioProcessor::kParamDryLevel, dryLin);
        setP (DisperserAudioProcessor::kParamWetLevel, wetLin);
    ...
    aw->enterModalState (true,
            ...
            [safeThis, aw, origDry, origWet] (int result)
        {
            ...
            if (result != 1)
            {
                ...
                setP (DisperserAudioProcessor::kParamDryLevel, origDry);
                setP (DisperserAudioProcessor::kParamWetLevel, origWet);
                safeThis->dualMixBar_.updateFromProcessor();
            }
        }),
        false);
...
juce::String DisperserAudioProcessorEditor::getVariationText() const
{
    const double v = juce::jlimit (0.0, 1.0, variationSlider.getValue());
    const int pctInt = (int) std::lround (v * 100.0);
    return juce::String (pctInt).toUpperCase() + "% VAR";
}

juce::String DisperserAudioProcessorEditor::getVariationTextShort() const
{
    const double v = juce::jlimit (0.0, 1.0, variationSlider.getValue());
    const int pctInt = (int) std::lround (v * 100.0);
    return juce::String (pctInt).toUpperCase() + "% VAR";
...
namespace
{
    ...
    constexpr const char* kVariationLegendFull  = "100% VAR";
    constexpr const char* kVariationLegendShort = "100% VAR";
    constexpr const char* kVariationLegendInt   = "100";
...
DisperserAudioProcessorEditor::buildVerticalLayout (int editorH, int biasY, bool ioExpanded)
{
    ...
    // Bars below toggle: 9 IO rows when expanded (IN, OUT, TILT, FILTER, PAN, MIX, LIM + MODE_ROW + INV_ROW), 8 main bars when collapsed.
...
void DisperserAudioProcessorEditor::updateCachedLayout()
{
    ...
    const juce::Slider* sliders[12] = { &freqSlider, &modSlider, &feedbackSlider, &amountSlider,
                                         &seriesSlider, &shapeSlider, &variationSlider, &styleSlider,
    ...
    for (int i = 0; i < 12; ++i)
    {
        if (! sliders[i]->isVisible())
        {
            ...
            if (i == 11 && dualMixBar_.isVisible())
            {
                ...
                cachedValueAreas_[11] = { valueX, y, juce::jmax (0, vw), kValueAreaHeightPx };
                continue;
            }
...
int DisperserAudioProcessorEditor::getTargetValueColumnWidth() const
{
    ...
    const int variationMaxW = juce::jmax (stringWidth (font, kVariationLegendFull),
                                          juce::jmax (stringWidth (font, kVariationLegendShort),
                                                      stringWidth (font, kVariationLegendInt)));
    ...
    const int maxW = juce::jmax (juce::jmax (amountMaxW, seriesMaxW),
                                 juce::jmax (juce::jmax (freqMaxW, shapeMaxW),
                                             juce::jmax (juce::jmax (variationMaxW, juce::jmax (feedbackMaxW, modMaxW)),
...
juce::Slider* DisperserAudioProcessorEditor::getSliderForValueAreaPoint (juce::Point<int> p)
{
    ...
    if (getValueAreaFor (variationSlider.getBounds()).contains (p))
        return &variationSlider;
...
void DisperserAudioProcessorEditor::mouseDown (const juce::MouseEvent& e)
{
    ...
    if (altButton.isVisible() && getAltLabelArea().contains (p))
    {
...
void DisperserAudioProcessorEditor::mouseDoubleClick (const juce::MouseEvent& e)
{
    ...
    if (auto* slider = getSliderForValueAreaPoint (p))
    {
        ...
        else if (slider == &variationSlider)  slider->setValue (kDefaultVariation, juce::sendNotificationSync);
...
void DisperserAudioProcessorEditor::paint (juce::Graphics& g)
{
...

    {
        ...
        {
            const int drawW = titleMaxW > 0 ? titleMaxW : titleArea.getWidth();
            g.drawText (titleText, titleArea.getX(), titleArea.getY(), drawW, titleArea.getHeight(), juce::Justification::left, false);
        }
    ...
    {
        const juce::String* fullTexts[12]  = { &cachedFreqTextHz, &cachedModTextFull,
                                                &cachedFeedbackTextFull, &cachedAmountTextFull,
                                                &cachedSeriesTextFull, &cachedShapeTextFull,
                                                &cachedVariationTextFull, &cachedStyleTextFull,
                                                &cachedInputTextFull, &cachedOutputTextFull,
                                                &cachedTiltTextFull, &cachedMixTextFull };
        const juce::String* shortTexts[12] = { &cachedFreqTextShort, &cachedModTextShort,
                                                &cachedFeedbackTextShort, &cachedAmountTextShort,
                                                &cachedSeriesTextShort, &cachedShapeTextShort,
                                                &cachedVariationTextShort, &cachedStyleTextShort,
                                                &cachedInputTextShort, &cachedOutputTextShort,
                                                &cachedTiltTextShort, &cachedMixTextShort };
...
void DisperserAudioProcessorEditor::resized()
{
    ...
    else
    {
        ...
        variationSlider.setBounds (horizontalLayout.leftX, mainTop + 6 * step, horizontalLayout.barW, verticalLayout.barH);

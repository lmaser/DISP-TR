...
void DisperserAudioProcessorEditor::openNumericEntryPopupForSlider (juce::Slider& s)
{
    ...
    if (auto* te = aw->getTextEditor ("val"))
    {
        ...
        else if (&s == &tiltSlider)    worstCaseText = "-6.0";
        ...
        {
            minVal = DisperserAudioProcessor::kTiltMin;
            maxVal = DisperserAudioProcessor::kTiltMax;
            maxDecs = 1;
            maxLen = 4;
        }
        else if (&s == &limThresholdSlider)
        {
            minVal = limThresholdSlider.getMinimum();
            maxVal = limThresholdSlider.getMaximum();
            maxDecs = 1;
            maxLen = 5;
        }
        else if (&s == &panSlider)
    ...
    aw->enterModalState (true,
        juce::ModalCallbackFunction::create ([safeThis, sliderPtr, aw] (int result) mutable
        {
            ...
            if (safeThis != nullptr && (sliderPtr == &safeThis->shapeSlider || sliderPtr == &safeThis->jitterSlider || sliderPtr == &safeThis->feedbackSlider || sliderPtr == &safeThis->mixSlider || sliderPtr == &safeThis->panSlider))
                v *= 0.01;
...
void DisperserAudioProcessorEditor::openFilterPrompt()
{
                        ...
                        promptFont, sideFont, toggleSide, margin] ()
    {
        ...
        // Keep side controls compact; the central value block carries the visual weight.
                             ...
                             juce::Label* slopeLabel, PromptBar* bar, int y)
        {
            ...
            // ── Middle zone: number + unit, dynamically sized, centred ──
    ...
    *layoutFn = layoutRows;
    ...
    preparePromptTextEditor (*aw, "lpFreq", scheme.bg, scheme.text, scheme.fg, promptFont, false);
    if (auto* te = aw->getTextEditor ("hpFreq"))
        updateFilterPromptPresentation (te, hpHzLabel, parseFilterPromptFrequency (te, hpHzLabel, hpFreq), true);
    if (auto* te = aw->getTextEditor ("lpFreq"))
        updateFilterPromptPresentation (te, lpHzLabel, parseFilterPromptFrequency (te, lpHzLabel, lpFreq), true);
    layoutRows();
                                                            ...
                                                            const juce::String& title)
{
    ...
    aw->enterModalState (true,
             ...
             amtParamId, spdParamId] (int result) mutable
        {
            ...
            if (juce::String (amtParamId) == DisperserAudioProcessor::kParamChaosAmtFilter)
...
DisperserAudioProcessorEditor::buildVerticalLayout (int editorH, int biasY, bool ioExpanded)
{
    ...
    auto fitStack = [&] (int stackSliders, int stackGaps)
    {
        ...
        while (stackHeight() > spaceForScale && gapY > 4)
            --gapY;

        while (stackHeight() > spaceForScale && barH > 14)
            --barH;
...
juce::Slider* DisperserAudioProcessorEditor::getSliderForValueAreaPoint (juce::Point<int> p)
{
    ...
    if (getValueAreaFor (jitterSlider.getBounds()).contains (p))
        return &jitterSlider;
    ...
    if (getValueAreaFor (tiltSlider.getBounds()).contains (p))
        return &tiltSlider;
...
void DisperserAudioProcessorEditor::mouseDoubleClick (const juce::MouseEvent& e)
{
    ...
    if (auto* slider = getSliderForValueAreaPoint (p))
    {
        ...
        else if (slider == &panSlider)        slider->setValue (DisperserAudioProcessor::kPanDefault, juce::sendNotificationSync);
...
void DisperserAudioProcessorEditor::paint (juce::Graphics& g)
{
...

    {
        ...
        if (versionW > 0)
            g.drawText (versionText, versionX, versionY, versionW, versionH,
...

    {
        ...
        if (altButton.isVisible())
        {
            ...
            drawToggleLegend (getMidiLabelArea(), chooseToggleLabel (midiButton, midiCR, "MIDI", "MIDI"), midiCR);
        }
...
void DisperserAudioProcessorEditor::resized()
{
    ...
    else
    {
        ...
        jitterSlider.setBounds (horizontalLayout.leftX, mainTop + 6 * step, horizontalLayout.barW, verticalLayout.barH);
